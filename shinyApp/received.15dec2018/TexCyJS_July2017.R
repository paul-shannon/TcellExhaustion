...
# tbl.nodes <- read.table(nodeAnnotationFile, sep="\t", header=FALSE, as.is=TRUE)
...
nodes.from.nodesFile <- tbl.nodes$Symbol
...
httpSetStyle(rcy, "styleCancer.js")
...
setNodeAttributes(rcy, "expression", tbl.nodes$Source, tbl.nodes[, expression.conditions[1]])

if (length(expression.conditions) == length(score.conditions)) { # and we assume they are matched
for (i in 1:length(expression.conditions)) {
     ...
     edgeTypes   <- tbl.edges$edgeType
     values      <- tbl.edges[, score.condition]
