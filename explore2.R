...
findAndDisplayMotifs <- function(tbl.regions, pwmMatchMinimumAsPercentage, source, trackName)
{
   ...
   pfms <- as.list(pfms.mouse) # as.list(c(pfms.human, pfms.mouse))
   ...
   if (length(tbl.motifs$motifName) > 0) {
...
makeModel <- function(trena, targetGene, targetGene.tss, tbl.motifs, mtx.rna, pcaMaxThreshold=1.0)
{
  ...
  tbl.regulatoryRegions <- subset(tbl.motifs, geneSymbol %in% tbl.geneModel$gene)   # 71 rows
  ...
  #browser()
  distance <- tbl.regulatoryRegions$motifStart - targetGene.tss
  direction <- rep("upstream", length(distance))
  direction[which(distance < 0)] <- "downstream"
  tbl.regulatoryRegions$distance.from.tss <- distance
  tbl.regulatoryRegions$id <- sprintf("%s.fp.%s.%06d.%s", targetGene, direction, abs(distance), tbl.regulatoryRegions$motifName)
 ...
 # tbl.tfBindingFreq <- as.data.frame(table(tbl.regulatoryRegions.strong$geneSymbol))
  ...
  list(model=tbl.geneModel, regions=tbl.regulatoryRegions)
...
simple.demo <- function()
{
   ...
   for (i in 1:nrow(HB.tbl.regions.atac)) {
     addBedGraphTrackFromDataFrame(tv,
                                   ...
                                   HB.tbl.regions.atac.toDisplay[i,],
   ...
   if (nrow(tbl.motifs) > 0) {
	   model <- makeModel(trena, targetGene, targetGene.tss, tbl.motifs, mtx.rna, pcaMaxThreshold=0.5)
           colnames(model$model) <- gsub(".", "", colnames(model$model), fixed=TRUE)  # another pending cyjs fix
	   # one option is to now examine model$model, keeping only those (for instance) with a
		# random forest score > 10 or ....
	   #model$model <- subset(model$model, rfscore > 10)
	   ...
	   models <- list(simple=model, simpleDup=model)  # works around a bug in cyjs, soon to be fixed!
...
hamids.new.mouse.stringent.motifs <- function(match.threshold)
{
   ...
   mouse.jaspar.pfms <- query(query(MotifDb, "jaspar2016"), "mmus")
